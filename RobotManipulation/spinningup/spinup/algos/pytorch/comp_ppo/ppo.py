...
def ppo(env_fn, ac, macro_buffer=None, *, task_id, seed=0, 
    ...
    def compute_loss_v(data, *, task_id):
        ...
        loss_v = ((ac.v(obs, task_id=task_id) - ret)**2).mean()
        return loss_v

    # Set up optimizers for policy and value function
    pi_optimizer = Adam(ac.pi.parameters(), lr=pi_lr, eps=1e-5)
    vf_optimizer = Adam(ac.v.parameters(), lr=vf_lr, eps=1e-5)
    ...
    def update(*, task_id):
        ...
        v_l_old = compute_loss_v(data, task_id=task_id).item()

        # Train policy with multiple steps of gradient descent
        for i in range(train_pi_iters):
            pi_optimizer.zero_grad()
            loss_pi, pi_info = compute_loss_pi(data, task_id=task_id)
        ...
        for i in range(train_v_iters):
            ...
            loss_v = compute_loss_v(data, task_id=task_id)
            loss_v.backward()
