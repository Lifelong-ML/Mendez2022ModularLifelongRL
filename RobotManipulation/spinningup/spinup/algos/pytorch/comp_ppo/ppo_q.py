...
def ppo_q(env_fn, ac, macro_buffer=None, *, task_id, seed=0, 
    ...
    def update(*, task_id):
        ...
        for i in range(train_pi_iters):
            batch_idx = np.random.permutation(local_steps_per_epoch)[:256 // num_procs()]
            data_batch = {k: v[batch_idx] for k, v in data.items() if k != 'obs'}
            data_batch['obs'] = {o_k: o_v[batch_idx] for o_k, o_v in data['obs'].items()}
