...
class PPOAlgo(BaseAlgo):
    ...
    def update_parameters(self, exps_list, task_id_list):
        # Collect experiences

        # integrate MTL and STL into single fn
        if not isinstance (exps_list, list):
            exps_list = [exps_list]
        ...
        for _ in range(self.epochs):
            ...
            for inds in self._get_batches_starting_indexes():
                ...
                mtl_value_loss = 0
                mtl_loss = 0
                for task_id, exps in zip(task_id_list, exps_list):
                ...
                mtl_entropy /= len(task_id_list)
                mtl_value /= len(task_id_list)
                mtl_policy_loss /= len(task_id_list)
                mtl_value_loss /= len(task_id_list)
                mtl_loss /= len(task_id_list)
                # Update actor-critic
