...
class ACModelBcqProgressCompress(nn.Module):
    def __init__(
        self,
        input_shape,
        num_actions,
    ...
    def create_kb(self):
        ...
        self.critic_kb_linear1 = nn.Linear(64, self.num_actions)
    ...
    def create_active(self):
        ...
        self.critic_active_linear1 = ActiveLinear(64, self.num_actions, activation=nn.Identity())
    ...
    def fc(self, x, task_id, return_bc=False):
        if self.use_kb:
            ...
            if return_bc:
                x_q = self.critic_kb(x)
                x_bc = self.actor_kb(x)
                return x_q, F.log_softmax(x_bc, dim=1), x_bc

            x_actor = self.actor_kb(x)
            x_critic = self.critic_kb(x).max(dim=1, keepdim=True)[0]
            ...
            if return_bc:
                x_q_active = self.critic_active_linear0(x_active, x_kb)
                x_q_kb = self.critic_kb_linear0(x_kb)
                x_q = self.critic_active_linear1(x_q_active, x_q_kb)

                x_bc_active = self.actor_active_linear0(x_active, x_kb)
                x_bc_kb = self.actor_kb_linear0(x_kb)
                x_bc = self.actor_active_linear1(x_bc_active, x_bc_kb)
                return x_q, F.log_softmax(x_bc, dim=1), x_bc
            ...
            x_critic = self.critic_active_linear1(x_critic_active, x_critic_kb).max(dim=1, keepdim=True)[0]
            # exit()
            return x_actor, x_critic
